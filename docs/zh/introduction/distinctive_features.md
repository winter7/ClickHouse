...
在一些列式数据库管理系统中(例如：InfiniDB CE and MonetDB) 不使用数据压缩。但是, 数据压缩在实现优异的存储系统中确实起着关键的作用。
...
在ClickHouse中，数据可以保存在不同的shard上，每一个shard都由一组用于容错的replica组成，查询可以并行的在所有shard上进行处理。这些对用户来说是透明的
